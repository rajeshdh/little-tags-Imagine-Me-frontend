...
export default function Search(props) {
  ...
  const { products, error, isLoading, filterCriteria } = useSelector(
    (state) => ({
  ...
  if (isLoading) {
    ...
    result = (
      <div className="flex mt-10 justify-center text-lg items-center w-full">
    ...
    result = (
      <div className="flex mt-10 justify-center items-center">
        <EmptyIcon className="mr-2" />{' '}
        <FormattedMessage id="noProduct" defaultMessage="No Result found" />
      </div>
    );
  } else {
    result = (
      <div className="py-3 mt-5 grid gap-4 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5">
        {products.map((product, index) => (
          <ProductCard key={`product_id_${index}`} details={product} />
        ))}
      </div>
    );
  }
  ...
  return (
    <div className="mx-3 mt-10 sm:mx-10 md:mx-24">
      <div className="flex text-lg font-semibold items-center justify-between">
        <div>
          <FormattedMessage
            id="searchResult"
            defaultMessage="Showing Results For {keyWord}"
            values={{
              keyWord: <span className="text-sp-heading-blue">{keyWord}</span>,
            }}
          />
        </div>
