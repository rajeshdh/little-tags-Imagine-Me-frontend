...
export default function FilterDrawer({ show, onClick, requestProducts }) {
  ...
  const generateRatingFilter = (CheckBox) => {
    ...
    for (let i = 5; i > 0; i--) {
      ratingCheckbox.push(
        <CheckBox key={`filter_star_rating_id_${i}`} value={`${i}`}>
          <StarRating rating={i} />{' '}
        </CheckBox>,
      );
    }
  ...
  return (
    <BackDrop show={show} onClick={onClick}>
      <div
        ...
        className={`absolute bg-white inset-y-0 right-0 max-w-xs w-full transition-transform duration-400 transform ${
          show ? 'translate-x-0' : 'translate-x-full'
        }`}
      >
        <div className="relative h-full pb-5 overflow-y-auto">
          <div
            className="float-right mr-2 text-4xl cursor-pointer"
            onClick={onClick}
          >
            &times;
          </div>
          <div className="mt-10 px-4">
            ...
            <div className="my-2">
              <FormattedNumber
                style="currency"
                value={filterCriteria.price[0] ?? price[0]}
                currency="INR"
              />{' '}
              -{' '}
              <FormattedNumber
                style="currency"
                value={filterCriteria.price[1] ?? price[1]}
                currency="INR"
              />
            </div>
            <div className="mx-3">
              <RangeSlider
                ...
                defaultValue={[
                  filterCriteria.price[0] ?? price[0],
                  filterCriteria.price[1] ?? price[1],
                ]}
                min={price[0]}
                max={price[1]}
                marks={{
            ...
            <div className="mt-9 mb-3 border-b border-gray-300"></div>
            ...
            <div className="mt-3 mb-3 border-b border-gray-300"></div>
            <div className="font-semibold tracking-wide uppercase mb-2">
        ...
        <div className="absolute bg-white p-2 inset-x-0 bottom-0">
          <button
            className="bg-sp-btn-primary text-white px-5 py-2 rounded float-right"
            name="filter_submit"
