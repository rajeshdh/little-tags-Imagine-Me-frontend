...
export default function BottomNavigation() {
  ...
  return <div className="fixed inset-x-0 bottom-0 bg-white z-30 border-t border-grey-400 shadow-2xl sm:hidden">
    <div className="flex justify-between items-center mx-3">
      ...
      <NavLink to="/cart" className="relative pt-3 pb-3 pr-3">
        <div className="absolute -top-0.5 -left-2.5 w-5 h-5 flex justify-center items-center text-white text-xs bg-sp-btn-primary rounded-full">{cartLength}</div>
        <CartIcon />
      </NavLink>
      <NavLink to="/login" className="pt-3 pb-3 pr-3">
        <LoginIcon />
      </NavLink>
    </div>
  </div>
