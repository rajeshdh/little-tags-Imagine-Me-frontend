...
function SearchButton({ handleClick }) {
  return (
    ...
    >
      ...
      >
        <path
          d="M37.3702 34.0723L27.4171 24.1191C29.3451 21.7377 30.5059 18.7116 30.5059 15.4158C30.5059 7.77708 24.2905 1.56165 16.6518 1.56165C9.01315 1.56165 2.79785 7.77702 2.79785 15.4158C2.79785 23.0545 9.01322 29.27 16.6519 29.27C19.9477 29.27 22.9737 28.1091 25.3551 26.1812L35.3081 36.1343C35.5925 36.4187 35.9658 36.5616 36.3392 36.5616C36.7125 36.5616 37.0859 36.4187 37.3702 36.1343C37.9404 35.5641 37.9404 34.6425 37.3702 34.0723ZM16.6519 26.3533C10.6203 26.3533 5.71451 21.4475 5.71451 15.4158C5.71451 9.38408 10.6203 4.47827 16.6519 4.47827C22.6835 4.47827 27.5893 9.38408 27.5893 15.4158C27.5893 21.4475 22.6834 26.3533 16.6519 26.3533Z"
          fill="#F9F9F9"
        />
      </svg>
    </button>
...
function CloseButton({ handleClick }) {
  return (
    <button
      type="button"
      className="font-bold py-2 px-4 rounded items-center justify-center"
      onClick={handleClick}
    >
      <svg
  ...
  return (
    <Downshift
      onChange={selection => {
        ...
        const path = `/search?q=${selection.value}`;
        history.push(path);
      }}
      ...
      {({
        ...
        <div {...getRootProps({}, { suppressRefError: true })}>
          <div className={`${isSearchOpen ? 'inline-block' : 'hidden'} `}>
            ...
            <aside
              className="absolute z-50 flex flex-col items-start w-96 bg-white border rounded-md shadow-xl animate-bottom-top"
              role="menu"
