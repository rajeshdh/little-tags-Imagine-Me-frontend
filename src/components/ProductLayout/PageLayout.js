...
export const ProductPageLayout = ({ children }) => (
  <section className="text-sp-text-default body-font overflow-hidden ">
    <div className="container px-5 pt-4 pb-24 mx-auto ">
      <div className="lg:w-4/5 mx-auto flex flex-wrap">{children}</div>
    </div>
  </section>
);
...
export const ProductHeader = ({ brandName, title }) => (
  <>
    <h2 className="text-sm title-font text-sp-text-default tracking-widest">
      {brandName}
    </h2>
    <h1 className="text-sp-text-default font-extrabold text-3xl title-font mb-1">
      {title}
    </h1>
  </>
);
...
export const ProductSection = ({ sectionClasses, children }) => {
  const classes =
    `lg:w-1/2 w-full lg:pl-10 lg:py-6 mt-6 lg:mt-0 ${sectionClasses}` || '';
  return <div className={classes}>{children}</div>;
};

export const Price = ({ price, currency }) => (
  <span className="title-font font-extrabold text-2xl text-sp-text-default">
...
export const LikeButton = () => (
  <button
    ...
    className=" w-12 h-12 p-0 inline-flex items-center justify-center ml-4 hover:text-sp-btn-primary bg-transparent border border-solid border-sp-btn-selected"
  >
    <svg
      fill="currentColor"
      strokeLinecap="round"
      ...
      strokeWidth="2"
      className="w-5 h-5 transform delay-100 hover:scale-150"
      viewBox="0 0 24 24"
    >
